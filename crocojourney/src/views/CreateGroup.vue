<template>
    <div class="text-start mt-5 mb-4 col-md-6 mx-auto" id="alertsDiv"></div>
        ...
        <div class="d-block h-25 mt-5">
            <div class="row">
                <div class="col px-5">
                    ...
                    <p ref="emptyText" class="text-muted text-center align-bottom">
                        Vous n'avez pour l'instant ajouté personne au groupe. Utilisez la liste de droite pour ajouter
                        des amis !
                    </p>
                    <ul
                        ref="addedList"
                        class="list-group list-group-flush overflow-y-auto mt-4"
                ...
                <div class="col px-5">
                    ...
                    <div class="overflow-y-auto d-inline-block mt-4" style="max-height: 200px">
                        <div class="list-group" ref="userlist"></div>
                    </div>
                </div>
            </div>
        </div>
...
export default defineComponent({
    ...
    methods: {
        ...
        async adduser(id) {
            ...
            array.forEach((child) => {
                if (
                    child.textContent.includes(addeduser.firstname) &&
                    child.textContent.includes(addeduser.lastname) &&
                    child.textContent.includes(addeduser.mail)
                ) {
                    childToDelete = child;
                }
            });
        ...
        async creategroup() {
            ...
            try {
                ...
                await API.requestLogged(
                    API.METHOD.POST,
                    '/groups/' + groupId + '/friends/',
                    JSON.stringify(friendsRequest),
                    API.CONTENT_TYPE.JSON
                );
                ...
                document.querySelector('#alertsDiv').innerHTML =
                    '<div class="alert alert-success alert-dismissible fade show" role="alert"><div><strong>Création réussie !</strong> Vous pouvez maintenant ajouter ce groupe à un trajet. <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div></div>';
            } catch (error) {
                if (error.status == 422)
                    document.querySelector('#alertsDiv').innerHTML =
                        '<div class="alert alert-danger alert-dismissible fade show" role="alert"><div><strong>Oups !</strong> Une erreur est survenue lors de la création du groupe (Un groupe de ce nom existe déjà). <br><button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button></div></div>';
                //console.log(error);
            }
        },
    },
